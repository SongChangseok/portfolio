...
import { transformTagData } from '@/lib/utils/chart-data';
import { getChartColor } from '@/lib/utils/chart-colors';
import { formatCurrency } from '@/lib/utils/formatters';
import type { Holding, StockTag, Tag } from '@/lib/types';
...
export function TagBreakdownChart({ holdings, stockTags, tags }: TagBreakdownChartProps) {
  if (tags.length === 0) return null;

  const data = transformTagData(holdings, stockTags, tags);
    ...
    >
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          ...
          >
            {data.map((item, index) => (
              <Cell key={index} fill={item.color ?? getChartColor(index)} />
            ))}
          </Pie>
